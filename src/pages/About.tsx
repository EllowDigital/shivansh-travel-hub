...
const values = [
  { icon: ShieldCheck, title: "Safety First", desc: "All vehicles regularly serviced and drivers fully verified." },
  { icon: Heart, title: "Customer Focused", desc: "Your comfort and satisfaction is our top priority." },
  { icon: Users, title: "Family Friendly", desc: "Catering to families, tourists, pilgrims, and corporates." },
  ...
  <div className="pt-14 sm:pt-16 lg:pt-[88px]">
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-10 sm:mt-12 max-w-3xl mx-auto">
          <div className="bg-card rounded-xl p-5 sm:p-6 border border-border/50 shadow-sm text-center">
            ...
            <h3 className="font-heading font-semibold text-foreground text-sm sm:text-base">Our Mission</h3>
            <p className="mt-2 text-xs sm:text-sm text-muted-foreground">To provide safe, reliable, and affordable travel services that exceed customer expectations every single time.</p>
          </div>
          <div className="bg-card rounded-xl p-5 sm:p-6 border border-border/50 shadow-sm text-center">
            <div className="mx-auto mb-3 w-12 h-12 rounded-full bg-accent flex items-center justify-center">
              <Globe className="h-6 w-6 text-secondary" />
            </div>
            <h3 className="font-heading font-semibold text-foreground text-sm sm:text-base">Our Vision</h3>
            <p className="mt-2 text-xs sm:text-sm text-muted-foreground">To become India's most trusted travel partner by connecting people to their dream destinations.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="section-padding bg-muted">
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {values.map((v, i) => (
            <div key={i} className="bg-card rounded-xl p-5 sm:p-6 text-center hover-lift shadow-sm border border-border/50">
              ...
              <h3 className="font-heading font-semibold text-foreground text-sm sm:text-base">{v.title}</h3>
              <p className="mt-2 text-xs sm:text-sm text-muted-foreground">{v.desc}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="section-padding bg-secondary text-center">
      <div className="container mx-auto">
        ...
        <p className="mt-2 text-secondary-foreground/90 text-sm sm:text-base">Contact us today for bookings and custom tour packages.</p>
        <div className="mt-6">
          <Link to="/contact"><Button variant="default" size="lg" className="text-sm sm:text-base">Contact Us</Button></Link>
        </div>
      </div>
    </section>
  </div>
