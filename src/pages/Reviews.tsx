...
const Reviews = () => (
  <div className="pt-14 sm:pt-16 lg:pt-[88px]">
    ...
    <section className="bg-accent py-8 sm:py-10">
      <div className="container mx-auto text-center">
        <div className="inline-flex flex-col items-center bg-card rounded-2xl px-8 py-6 shadow-md border border-border/50">
          <p className="font-heading text-4xl sm:text-5xl font-extrabold text-foreground">{avgRating}</p>
          <div className="flex gap-0.5 mt-2">
            {[1, 2, 3, 4, 5].map(s => (
              <Star key={s} className={`h-5 w-5 ${s <= Math.round(Number(avgRating)) ? "fill-secondary text-secondary" : "text-muted-foreground/30"}`} />
            ))}
          </div>
          <p className="text-sm text-muted-foreground mt-2">Based on {allReviews.length}+ verified reviews</p>
        </div>
      </div>
    </section>
    ...
    <section className="section-padding bg-background">
      <div className="container mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {allReviews.map((r, i) => (
            <div key={i} className="bg-card rounded-xl p-5 sm:p-6 border border-border/50 shadow-sm hover-lift relative">
              ...
              <p className="text-muted-foreground text-xs sm:text-sm leading-relaxed italic">"{r.text}"</p>
              <div className="mt-4 pt-3 border-t border-border/50 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-foreground text-xs sm:text-sm">{r.name}</p>
                    <p className="text-[10px] text-muted-foreground">{r.city}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-[10px] bg-accent text-accent-foreground px-2 py-0.5 rounded-full">{r.trip}</span>
    ...
    <section className="section-padding bg-primary text-center">
      <div className="container mx-auto">
        <h2 className="font-heading text-2xl sm:text-3xl font-bold text-primary-foreground">Join Our Happy Customers</h2>
        <p className="mt-2 text-primary-foreground/80 text-sm sm:text-base max-w-lg mx-auto">
          Book your ride today and experience why thousands trust Shivansh Travels.
        </p>
